  ...
  <div class="preview-overlay" @click.self="emit('close')">
    <div class="preview-container">
      ...
      <template v-if="urls.length > 1">
        <button class="nav-btn nav-prev" :disabled="currentIndex === 0" @click="prev">
          <ion-icon name="chevron-back-outline" />
        </button>
        <button class="nav-btn nav-next" :disabled="currentIndex === urls.length - 1" @click="next">
      ...
      <div class="action-bar">
        <ion-button fill="clear" color="light" @click="handleDownload" :disabled="downloading">
          <ion-icon slot="start" :name="downloading ? 'hourglass-outline' : 'download-outline'" />
          {{ downloading ? '保存中...' : '下载' }}
        </ion-button>
        <ion-button fill="clear" color="light" @click="showSharePanel = true">
