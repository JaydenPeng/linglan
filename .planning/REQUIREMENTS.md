...
- [ ] **CONF-01**: 用户可以在设置页面输入并保存即梦 AK/SK
- [ ] **CONF-02**: 用户可以在设置页面输入并保存可灵 AK/SK
- [ ] **CONF-03**: 用户可以在设置页面查看当前已配置的密钥状态（已配置/未配置，不显示明文）

### Image Generation（生图）

- [ ] **IMG-01**: 用户可以通过文字提示词发起生图任务（文生图）
- [ ] **IMG-02**: 用户可以上传参考图片并结合提示词发起生图任务（图生图）
- [ ] **IMG-03**: 用户可以配置图片参数（宽度、高度、宽高比）
- [ ] **IMG-04**: 用户可以配置 force_single 强制单图输出
...
- [ ] **TASK-01**: 用户可以实时查看任务状态（已提交/处理中/成功/失败）
- [ ] **TASK-02**: 系统支持多个任务并行执行，轮询直到所有任务完成
- [ ] **TASK-03**: 用户可以对失败的任务发起重试
- [ ] **TASK-04**: 用户可以取消进行中的任务（停止轮询）
